...
export function addApiMonitoring(
  ...
  const apiGatewayName = `${fortunasBet}-Api-${stage}`;
  const apiStageName = api.deploymentStage.stageName;

  // Create SNS Topic for alarm notifications
  const alarmTopic = new sns.Topic(
    scope,
    `${fortunasBet}-ApiAlarmTopic-${stage}`,
    {
      topicName: `${fortunasBet}-ApiAlarmTopic-${stage}`,
      displayName: `${fortunasBet} API Alarm Topic (${stage})`,
    },
  );
  ...
  const alarm4xx = new cloudwatch.Alarm(
    scope,
    `${fortunasBet}-Api-4XXRateAlarm-${stage}`,
    {
      alarmName: `${fortunasBet}-Api-4XXRateAlarm-${stage}`,
      metric: api4xxRate,
      threshold: 50,
      evaluationPeriods: 6, // 6 x 5min = 30min
      datapointsToAlarm: 6, // all 6 periods must breach
      treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
      alarmDescription: `${fortunasBet}-Api-4XXRateAlarm-${stage}: Alarm if 4XX error rate exceeds 50% for 30 minutes on API Gateway (${stage})`,
      comparisonOperator:
  ...
  const alarm5xx = new cloudwatch.Alarm(
    scope,
    `${fortunasBet}-Api-5XXAlarm-${stage}`,
    {
      alarmName: `${fortunasBet}-Api-5XXAlarm-${stage}`,
      metric: api5xxMetric,
      threshold: 1,
      evaluationPeriods: 1,
      datapointsToAlarm: 1,
      treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
      alarmDescription: `${fortunasBet}-Api-5XXAlarm-${stage}: Alarm if any 5XX errors occur on API Gateway (${stage})`,
      comparisonOperator:
        cloudwatch.ComparisonOperator.GREATER_THAN_OR_EQUAL_TO_THRESHOLD,
      actionsEnabled: true,
    },
